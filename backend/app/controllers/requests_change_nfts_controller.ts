...
export default class RequestsChangeNftsController {
  ...
  async exchangeNFT(ctx: HttpContext) {
    ...
    const USER_HAVE_NFT = await db.from('have_nfts').where('id_nft', nftIdExchange).select('id_minter')
  ...
  async changeStatusRequest(ctx: HttpContext) {
    try {
      ...
      const userIdHaveNft = await db.from('have_nfts').where('id_nft', request.nft_id).select('id_minter')
      const userHaveNft = await User.find(userIdHaveNft[0]['id_minter'])
      ...
      if (isApproved) {
        ...
        console.log('Request approved'+user.id)
        console.log('Request approved'+userHaveNft.id)

        await NotificationService.createNotificationExchange(user, 10, request.nft_id_minter_would, userHaveNft)
        await NotificationService.createNotificationExchange(user, 12, request.nft_id_minter_would, userHaveNft)
      } else {
        await NotificationService.createNotificationExchange(user, 13, request.nft_id_minter_would, userHaveNft)
        await NotificationService.createNotificationExchange(user, 14, request.nft_id_minter_would, userHaveNft)
      }
