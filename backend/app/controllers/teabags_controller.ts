...
import {generateRandomImageName, uploadBase64ImageToAzureStorage} from "#controllers/user_controller";
...
export default class TeabagsController {
  ...
  async createTeaBag(ctx: HttpContext) {
    ...
    await db
      .table('followers')
      .insert({ id_follower: USER_CONNECT.id, id_followed: teaBag1.id })
