  ...
  async getNFTsFeed(ctx: HttpContext) {
    try {
      ...
      const nftsWithDetails = await Promise.all(nfts.map(async (nft) => {
        const likeCount = await db.from('like_nfts')
            .where('id_nft', nft.id)
            .count('* as totalLikes')
            .first();
        ...
        //   return ctx.response.status(404).json({ message: 'User not found' });
        // }
        // const isLiked = await db.from('like_nfts')
        //     .where('id_nft', nft.id)
        //     .andWhere('user_id', user?.id)
        //     .first();

        const comments = await db.from('commentaries')
            .where('id_nft', nft.id)
            .select('*')
            .innerJoin('users', 'users.id', 'users.username');

        const minterInfo = await db.from('have_nfts')
            .where('id_nft', nft.id)
            .select('id_minter')
            .first();
        ...
        return {
          nft: nft.toJSON(), // Assuming toJSON() returns all the NFT fields
          username: minter.username,
          mint: likeCount.totalLikes,
          isLiked: Boolean(0),
