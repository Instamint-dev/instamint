...
const ModalPurchaseNFT = ({ setOpen, nft }: ModalPurchaseNFTProps) => {
    ...
    const handlePurchase = () => {
        if (parseFloat(offerPrice) <= (nft?.nft.price||0)&&parseFloat(offerPrice)>0) {
            ...
            setError(t("The price must be less than or equal to the NFT price"))
        }
    }
        ...
        <div className="fixed inset-0 z-50 overflow-auto bg-smoke-light flex">
            <div className="relative p-8 bg-white w-128 h-128 m-auto flex-col flex rounded-lg">
                <header className="flex items-center justify-between w-full">
                    ...
                    <h2 className="flex-1 text-lg font-semibold text-center">{t("Purchase")}</h2>
                ...
                {modalConfirm && (
                    <ModalConfirm onClose={() => {setModalConfirm(false)}} onConfirm={handleConfirm} title={t("Confirm Purchase")} message={t("Do you want to make a purchase request for this NFT at $?", {offerPrice : offerPrice})} show={modalConfirm} />
                )}
